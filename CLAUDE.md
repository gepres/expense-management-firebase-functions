...
**Paquete compartido `@gastos/expense-ai`** (single source of truth con `gastos-backend`): la lógica IA pura — config de modelos (`models.ts` es un adapter delgado), prompts de extracción, parsers y el **ranking de clasificación** (`InferenceService` = adapter, ver §3) — vive en `D:\PROYECTOS\gepres\gastos-expense-ai`. Se consume vendoreada como `vendor/gastos-expense-ai-<v>.tgz` (dep `file:`, commiteada → Firebase la sube en su build aislado). **No edites prompts/modelos/ranking en este repo**: editá el paquete, `npm test`, `npm run sync` (compila+empaqueta+copia el `.tgz` a ambos repos), luego `npm i` acá. Subir versión en el `package.json` del paquete si cambia el contrato.
