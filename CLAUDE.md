...
src/services/
  anthropic.service.ts                ← parseExpenseMessage + extractReceiptData (Vision). Prompts/parsers de `@gastos/expense-ai`
  transcription.service.ts            ← Whisper, escribe a tmpfile y limpia
  inference.service.ts                ← ADAPTER delgado: delega el ranking 1→6 a `@gastos/expense-ai` (classifyExpense); inyecta Firestore + learning_log + LLM. API pública intacta
  ...
  learning-log.service.ts             ← bitácora por usuario (queryRelevant/append/feedback). El doc se construye con `buildLearningLogDoc` de @gastos/expense-ai (schema single-source con backend); tokenize/overlap re-exportados del paquete
  ...
  quota.service.ts                    ← checkQuota() pre-chequeo cuota IA (Fase 2, solo lectura, best-effort)
...
- **Saldo (Opción A — desacople):** el web app/backend es **dueño único** del saldo y del ledger. Este bot **NO** gestiona saldo ni `movements`: `saveExpense` solo escribe el `expense` contra la cuenta canónica (NO transacción con movement/saldo — el invariante viejo de `db.runTransaction()` quedó obsoleto). `resolveActiveAccount` resuelve desde la colección **canónica top-level `accounts`** (`saldo` derivado = `bankBalance + cashBalance`, solo lectura). Comandos `saldo`/`saldos` = lectura canónica (`AccountService.listCanonical`, NO `listByUser` legacy). `ingreso`/`transferir`/`movimientos` se **retiraron del bot** (responden derivando a la app) — no reintroducir escritura de ledger aquí. `MovementService` y `users/{uid}/{accounts,movements}` legacy fueron eliminados/orfanados. **Contrato de proyección de saldo (cross-repo, 2026-05-18):** `saveExpense` escribe `balanceApplied: false` en cada expense (marcador POSITIVO, metadata — NO toca saldo, Opción A intacta). El backend (`gastos-backend`, dueño del ledger) tiene un cron (`ExternalBalanceCron`, cada 1 min) que toma `expenses where balanceApplied == false`, debita el sub-saldo correcto (`metodoPago === 'efectivo'` → `cashBalance`/bolsillo; resto → `bankBalance`) en una tx idempotente y pone `balanceApplied: true`. **No quitar ni condicionar el `balanceApplied: false`** (sin él, un gasto efectivo por WhatsApp no descuenta del bolsillo — bug 2026-05-18). Solo el bot escribe ese campo; web/import/programados no lo tienen → el cron no los re-debita. Pendiente conocido: borrar/corregir el último gasto por WhatsApp aún no revierte saldo (mismo desacople; fuera de alcance de este fix).
...
- **Clasificación (`InferenceService.classify`):** el ranking vive en **`@gastos/expense-ai`** (`classifyExpense`, single source of truth con `gastos-backend`); `inference.service.ts` es un **adapter** que inyecta categorías (Firestore), `learning_log` (mapeado a `HistoryEntry`) y la llamada LLM. Orden estricto `suggestions_ideas` → nombre subcategoría → nombre categoría → historial (`learning_log`, solape `tokenOverlap` ≥ `MIN_HISTORY_OVERLAP`, prioriza `user_correction`) → **LLM acotado a la taxonomía** (paso 5: reusa el hint libre o `AnthropicService.classifyAgainstTaxonomy`, solo en miss de 1–4) → `sin_clasificar` (`needsClassification: true`). Pasos 1–3 match por **palabra/frase completa** (`phraseMatches`), nunca `includes` substring; el LLM (paso 5) nunca inventa categorías fuera de las del usuario. Devuelve `matchedTerm`/`matchedLevel` (`suggestion|subcategory|category|history|llm|default`) para auditoría. **No edites el ranking acá** — se edita en el paquete y se propaga con `npm run sync` (ver §6).
...
- **Retry policy:** máximo 3 intentos. El `catch` de `handleQueueDoc` incrementa `retryCount` y deja el doc en `pending`; tras el 3er fallo → `failed` + se notifica al usuario. **`onDocumentCreated` solo dispara en la creación**: quien reprocesa los `pending` es **`reprocessPendingQueue`** (`onSchedule`, cada 2 min) — claim transaccional `pending→processing`, reinvoca `handleQueueDoc`, recupera `processing` huérfano (>10 min) y emite `whatsapp_queue_stuck` si `retryCount > 3`. No reintroducir lógica que asuma que el trigger re-toma `pending` solo. Errores transitorios de IA (429/408/5xx/red) se reintentan con backoff (`src/utils/retry.ts`, `withRetry`) y, si persisten, se **propagan** (el item queda `pending`, no se traga como "no pude procesar"). Dedupe de Twilio: `twilioWebhook` encola con id determinístico = `MessageSid` (`.doc(sid).create()`, `ALREADY_EXISTS` → ignora); el caveat viejo del `.add()` (2 docs por reintento) ya no aplica.
...
- **Observabilidad:** **dos** superficies de alerta (la policy las cubre con `combiner: OR`), no espolvorear logs de alerta en cada `catch`: (1) `onWhatsAppQueueFailed` (`onDocumentUpdated` sobre `whatsapp_queue`) capta cualquier transición a `failed` y emite `logger.error("ALERT whatsapp_queue_failed", {event:"whatsapp_queue_failed", …})`; (2) `reprocessPendingQueue` emite `logger.error("ALERT whatsapp_queue_stuck", {event:"whatsapp_queue_stuck", …})` para `pending` con `retryCount > 3` que no progresan. Alert policy (Cloud Logging): filtros `jsonPayload.event="whatsapp_queue_failed"` **y** `…="whatsapp_queue_stuck"` (`resource.type="cloud_run_revision" severity=ERROR`) — **versionada en `ops/alert-policy.json`**; el deploy NO la crea, aplicarla con `ops/README.md` (`gcloud` + canal). Si renombras un `event`, actualizar el filtro acá, en `ops/alert-policy.json`, `ops/README.md` y `docs/FEATURES.md`.
...
- **Consumo IA (Fase 1, tracking):** todas las llamadas IA del bot registran consumo vía `recordUsage()` (`src/services/usage.service.ts`), mismo esquema Firestore que el backend (`aiUsageEvents` + `aiUsageMonthly/{uid}_{YYYY-MM}` top-level + `aiUsageAppMonthly`, `repo:"functions"`). Por decisión de producto **todo lo del bot es `scope:"user"`** (el call site pasa `{ userId: user.id }`). Es **best-effort**: nunca lanza, jamás rompe el procesamiento. Las tarifas se estiman por env (`AI_PRICE_*`); Whisper estima segundos por tamaño de buffer (heurística). Escribe vía Admin SDK (no depende de rules). **Fase 2 (enforcement):** `checkQuota()` (`quota.service.ts`) + helper `aiQuotaBlocked()` en `index.ts` se ejecutan ANTES de los 3 caminos con IA (imagen, audio, fallback LLM de texto). Si excede el límite del rol (env `AI_QUOTA_*`, mismo cálculo y doc que el backend; admin ilimitado) → responde por WhatsApp con la fecha de reinicio y cierra el item `completed` (sin retry). Los comandos/queries/regex NO pasan por el chequeo → siguen funcionando aunque el usuario esté sin cuota. `checkQuota` es best-effort: si falla la lectura, NO bloquea.
...
**Var no-secreta requerida en prod:** `TZ=America/Lima` (en `.env`, bundled al deploy). Cloud Run corre en **UTC**; sin esto, `resolveQueryPeriod` (hoy/ayer/semana/mes), `parseDateFromText` y `parseFecha` calculan límites de día en UTC y, tras las 19:00 hora Perú, "hoy" salta al día siguiente y **excluye los gastos del día peruano** (bug real 2026-05-16). Perú = UTC-5 fijo. Doble protección: además `src/utils/timezone.ts` (side-effect, importado **primero** en `index.ts`). Verificable: `TZ=UTC node -e "...resolveQueryPeriod('hoy')..."` vs `TZ=America/Lima`.
...
**Modelos Anthropic por tier (no-secreto, opcional):** `ANTHROPIC_MODEL_PRIMARY` (parse principal de texto/voz, default `claude-sonnet-4-6`), `ANTHROPIC_MODEL_VISION` (OCR de comprobantes; **aislado de PRIMARY** para poder bajarlo a Haiku ~3× más barato sin afectar el parse — default del paquete `claude-sonnet-4-6`, en **prod `claude-haiku-4-5`**) y `ANTHROPIC_MODEL_HELPER` (fallbacks acotados, default `claude-haiku-4-5`). Resueltos en `src/config/models.ts` vía `modelParams(tier)` — **único lugar** que decide modelo + `thinking`/`output_config`. Invariante: `output_config.effort` solo se manda si el modelo lo soporta (Sonnet 4.6+/Opus 4.5+); Haiku 4.5 y Sonnet ≤4.5 devuelven **400** con `effort`. Default conservador: modelo no reconocido → sin `effort`. Migrado desde `claude-sonnet-4-20250514` (deprecado, se retira 2026-06-15). Audio: `OPENAI_MODEL_TRANSCRIBE` (default `gpt-4o-mini-transcribe`, resuelto en `transcribeModel()` del mismo `models.ts`; OpenAI, no Claude — Whisper legacy reemplazado). Las 3 vars `*_MODEL_*` son no-secretas (`.env`, bundled); cambiar modelo no toca código pero sí requiere redeploy.

**Paquete compartido `@gastos/expense-ai`** (single source of truth con `gastos-backend`): la lógica IA pura — config de modelos (`models.ts` es un adapter delgado), prompts de extracción, parsers, el **ranking de clasificación** (`InferenceService` = adapter, ver §3) y el **schema del `learning_log`** (`buildLearningLogDoc` — `append` lo usa, ver §2; la colección Firestore es compartida con el web, el doc DEBE ser byte-compatible) — vive en `D:\PROYECTOS\gepres\gastos-expense-ai`. Se consume vendoreada como `vendor/gastos-expense-ai-<v>.tgz` (dep `file:`, commiteada → Firebase la sube en su build aislado). **No edites prompts/modelos/ranking en este repo**: editá el paquete, `npm test`, `npm run sync` (compila+empaqueta+copia el `.tgz` a ambos repos), luego `npm i` acá. Subir versión en el `package.json` del paquete si cambia el contrato.
...
- **`voucherType`:** `ExpenseData.voucherType` es **opcional**; los parsers (`parseExpenseMessage`) ya **no** lo setean. Lo resuelve siempre `InferenceService.inferVoucherType` en `finalizeAndRegisterExpense` y se pasa a `saveExpense` (que además aplica `?? "boleta"` defensivo). No reintroducir defaults muertos en los parsers (§ A.1, limpiado 2026-05-16).
...
- **Caché in-instance de taxonomía (rec. #5):** `InferenceService.getCategories`/`getPaymentMethods` cachean por `userId` con TTL **60 s** (`src/utils/ttl-cache.ts`). Consistencia **eventual**: un alta de categoría/método en la app web tarda hasta 60 s en verse por WhatsApp. **No asumir lectura fresca por mensaje** ni escribir tests que esperen efecto inmediato tras un cambio en otra colección. No se cachea `[]` (distingue "sin datos" de fallo de lectura). El caché es por instancia y efímero.
- **Reprocesador / dedupe (rec. #1/#3):** `onDocumentCreated` solo dispara en la creación → quien reprocesa `pending` es `reprocessPendingQueue` (`onSchedule`, §3 Retry policy). `twilioWebhook` encola con id determinístico = `MessageSid` (`.doc(sid).create()`); el caveat viejo del `.add()` (2 docs por reintento de Twilio) **ya no aplica**. No reintroducir `.add()` ahí ni lógica que asuma que el trigger re-toma `pending` solo.
- **Zona horaria:** todo cálculo de fecha asume hora Perú vía `TZ=America/Lima` (ver §6). `expense.fecha` se guarda como `Timestamp` (instante absoluto, correcto); el filtrado por día/mes depende de que el runtime esté en Lima. No reintroducir `new Date()` asumiendo UTC ni hardcodear offsets — confiar en `TZ`.
